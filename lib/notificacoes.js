...
// Chave pública VAPID (par da privada configurada na Vercel) — identifica
// o NoctaLev junto ao serviço de push do navegador.
...
// ============================================================
// PUSH REMOTO — a correção do "lembrete que não chegava".
// Inscreve este aparelho no serviço de push do navegador e salva
// o "endereço de entrega" na nuvem. Com isso, o servidor consegue
// entregar o lembrete das 08:30 e das 21:30 MESMO COM O APP FECHADO.
// Chamar sempre que a permissão for concedida e na abertura do app.
// ============================================================
...
export async function ativarPush(userId) {
  try {
    ...
    if (statusPermissao() !== "granted") return false;
    ...
    await fetch("/api/push/salvar", {
      ...
      body: JSON.stringify({
        ...
        userId: userId || null,
        nome: s.perfil?.nome || null,
        horaCheckin: s.config?.lembreteCheckin || "08:30",
        horaRitual: s.config?.lembreteRitual || "21:30",
      }),
    });
    return true;
  } catch (e) {
    console.warn("ativarPush:", e?.message);
    return false;
  }
}
  ...
  try {
    ...
    if (sub) {
      await fetch("/api/push/salvar", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ remover: true, endpoint: sub.endpoint }),
      });
