...
export const DEFAULT_CONFIG = {
  ...
  checkoutFase2: "https://pay.cakto.com.br/3ep394x_1108824",
  checkoutFase3: "https://pay.cakto.com.br/t2tvh6v",
...
function estadoInicial() {
  return {
    ...
    fotoPerfil: null,               // dataURL JPEG da foto de PERFIL (avatar — independente da foto de antes)
    ...
    notasAdmin: [],
    conquistasNaoVistas: [], // fila de conquistas para celebrar (3.1)
    misturaRenovadaEm: null, // última vez que remontou a mistura (4.1)
    fotoAgora: null,         // dataURL da foto "agora" (3.7 — comparação)
    fotoAgoraEm: null,
    marcosVistos: [],        // marcos já celebrados: [7, 14, 21, 30] (3.8)
    semanasVistas: [],       // resumos semanais já vistos: ["2026-W35"] (3.3)
  };
}

// ===== 5.1 — proteção de dados: backup + recuperação automática =====
// Cada save também grava uma cópia de segurança. Se o dado principal
// corromper (celular desligou gravando, storage cheio etc.), o load
// recupera do backup automaticamente — a usuária nem percebe.
const KEY_BK = KEY + "_bk";

function parseEstado(raw) {
  const s = { ...estadoInicial(), ...JSON.parse(raw) };
  if (!s || typeof s !== "object" || Array.isArray(s)) throw new Error("estado inválido");
...
export function load() {
  ...
  // 1ª tentativa: dado principal
  try {
    const raw = localStorage.getItem(KEY);
    if (raw) return parseEstado(raw);
  } catch {}
  // 2ª tentativa: backup (recuperação silenciosa)
  try {
    const bk = localStorage.getItem(KEY_BK);
    if (bk) {
      const s = parseEstado(bk);
      try { localStorage.setItem(KEY, bk); } catch {}
      console.warn("NoctaLev: dados recuperados do backup local");
      return s;
    }
  } catch {}
...
export function save(s) {
  if (typeof window !== "undefined") {
    try {
      ...
      // backup recebe o valor BOM anterior antes de sobrescrever o principal
      const atual = localStorage.getItem(KEY);
      if (atual) { try { localStorage.setItem(KEY_BK, atual); } catch {} }
      localStorage.setItem(KEY, json);
    } catch (e) {
      // storage cheio (fotos grandes): tenta salvar sem as fotos para
      // nunca perder rituais/check-ins/pontos
      try {
        const magro = { ...s, fotoAntes: null, fotoAgora: null, fotoPerfil: null };
        localStorage.setItem(KEY, JSON.stringify(magro));
        console.warn("NoctaLev: storage cheio — salvo sem fotos", e?.message);
      } catch {}
    }
  }
  return s;
...
export function resetAll() {
  if (typeof window !== "undefined") {
    ...
    localStorage.removeItem(KEY_BK);
  }
}
...
function darConquista(s, tipo) {
  if (!s.conquistas[tipo]) {
    ...
    s.pontos += 20;
    // 3.1 — entra na fila de celebração (modal com confete na Home)
    s.conquistasNaoVistas = [...(s.conquistasNaoVistas || []), tipo];
    return true;
  }
  return false;
}

// 3.1 — marca a próxima conquista da fila como vista (retorna o estado salvo)
export function verConquista(s) {
  const fila = s.conquistasNaoVistas || [];
  s.conquistasNaoVistas = fila.slice(1);
  return save(s);
}
...
// ---------------- 3.4 — Níveis com títulos ----------------
export const NIVEIS = [
  { min: 0,   nome: "Iniciante",        emoji: "🌱" },
  { min: 80,  nome: "Constante",        emoji: "🌙" },
  { min: 200, nome: "Dedicada",         emoji: "💫" },
  { min: 380, nome: "Guerreira do Sono", emoji: "🔥" },
  { min: 600, nome: "Rainha do Sono",   emoji: "👑" },
];

export function nivelAtual(s) {
  const pts = s.pontos || 0;
  let i = 0;
  for (let k = 0; k < NIVEIS.length; k++) if (pts >= NIVEIS[k].min) i = k;
  const atual = NIVEIS[i];
  const prox = NIVEIS[i + 1] || null;
  const pct = prox ? Math.min(100, Math.round(((pts - atual.min) / (prox.min - atual.min)) * 100)) : 100;
  return { ...atual, indice: i, pontos: pts, proximo: prox, pct, faltam: prox ? prox.min - pts : 0 };
}

// ---------------- 3.5 — progresso rumo às conquistas bloqueadas ----------------
export function progressoConquistas(s) {
  ...
  const kg = pesoPerdido(s);
  return {
    tres_noites:     { atual: Math.min(calcStreak(s), 3), meta: 3, unid: "noites" },
    uma_semana:      { atual: Math.min(Object.keys(s.rituais).length, 7), meta: 7, unid: "rituais" },
    dorminhoca:      { atual: Math.min(boas, 5), meta: 5, unid: "noites boas" },
    kiwi_lover:      { atual: Math.min(nKiwi, 5), meta: 5, unid: "kiwis" },
    jantar_blindado: { atual: Math.min(nBanana, 7), meta: 7, unid: "jantares" },
    primeiro_kg:     { atual: Math.min(kg, 1), meta: 1, unid: "kg", decimais: 1 },
  };
}

// ---------------- 3.3 — resumo semanal "Sua semana" ----------------
function chaveSemana(dataISO) {
  // semana termina no domingo; chave = data do domingo
...
export function resumoSemanal(s) {
  // mostra no domingo (ou depois) o resumo da semana que terminou
  if (!s.receitaPreparadaEm) return null;
  const hoje = hojeSP();
  const d = new Date(hoje + "T12:00:00Z");
  const dow = d.getUTCDay();
  if (dow !== 0) return null; // só aparece no domingo
  const chave = hoje; // domingo de hoje
  if ((s.semanasVistas || []).includes(chave)) return null;

  // janela: segunda a domingo (últimos 7 dias terminando hoje)
  const ini = new Date(d); ini.setUTCDate(ini.getUTCDate() - 6);
  const iniISO = ini.toISOString().slice(0, 10);
  const noPeriodo = (data) => data >= iniISO && data <= hoje;

  const rituais = Object.keys(s.rituais).filter(noPeriodo).length;
  const checkins = Object.entries(s.checkins).filter(([data]) => noPeriodo(data));
  if (rituais === 0 && checkins.length === 0) return null; // semana vazia — não mostra

  // sono médio da semana vs semana anterior
  const mediaSono = checkins.length ? checkins.reduce((t, [, c]) => t + c.sono, 0) / checkins.length : null;
  const iniPrev = new Date(ini); iniPrev.setUTCDate(iniPrev.getUTCDate() - 7);
  const iniPrevISO = iniPrev.toISOString().slice(0, 10);
  const prevCheckins = Object.entries(s.checkins).filter(([data]) => data >= iniPrevISO && data < iniISO);
  const mediaPrev = prevCheckins.length ? prevCheckins.reduce((t, [, c]) => t + c.sono, 0) / prevCheckins.length : null;
  const sonoDelta = mediaSono != null && mediaPrev != null ? Math.round(((mediaSono - mediaPrev) / 5) * 100) : null;

  // peso: primeiro vs último registro da semana
  const pesosSemana = checkins.filter(([, c]) => c.peso != null).map(([data, c]) => ({ data, peso: c.peso }))
    .sort((a, b) => a.data.localeCompare(b.data));
  const pesoDelta = pesosSemana.length >= 2 ? pesosSemana[0].peso - pesosSemana[pesosSemana.length - 1].peso : null;

  return { chave, rituais, checkins: checkins.length, mediaSono, sonoDelta, pesoDelta };
}

export function marcarSemanaVista(s, chave) {
  s.semanasVistas = [...(s.semanasVistas || []), chave];
  return save(s);
}

// ---------------- 3.8 — marcos da jornada (dias 7/14/21/30) ----------------
export const MARCOS = {
  7:  { emoji: "🎉", titulo: "1 semana de protocolo!", texto: "Sete noites cuidando de você. O hábito está se formando — e seu corpo já percebeu." },
  14: { emoji: "🌟", titulo: "2 semanas completas!", texto: "Duas semanas de constância. É exatamente assim que os resultados se consolidam." },
  21: { emoji: "💎", titulo: "21 dias — virou hábito!", texto: "Dizem que 21 dias formam um hábito. O seu ritual noturno agora faz parte de você." },
  30: { emoji: "👑", titulo: "1 mês de jornada!", texto: "Um mês inteiro de dedicação. Olhe para trás e veja o quanto você evoluiu. Orgulho define!" },
};

export function marcoPendente(s) {
  if (!s.receitaPreparadaEm) return null;
  const dia = diaProtocolo(s);
  const vistos = s.marcosVistos || [];
  const alvo = [30, 21, 14, 7].find((m) => dia >= m && !vistos.includes(m));
  return alvo ? { dia: alvo, ...MARCOS[alvo] } : null;
}

export function marcarMarcoVisto(s, dia) {
  s.marcosVistos = [...(s.marcosVistos || []), dia];
  s.pontos += 15; // bônus do marco
  return save(s);
}

// ---------------- 3.2 — Noite de Perdão (proteção de streak) ----------------
// O calcStreak já cobre 1 buraco a cada 7 dias; aqui deixamos isso VISÍVEL.
export function estadoPerdao(s) {
  const dias = new Set([...Object.keys(s.checkins), ...Object.keys(s.rituais)]);
  if (dias.size === 0) return { disponivel: true, usadoEm: null };
  let cursor = hojeSP();
  if (!dias.has(cursor)) {
    const d = new Date(cursor + "T12:00:00Z"); d.setUTCDate(d.getUTCDate() - 1);
    cursor = d.toISOString().slice(0, 10);
  }
  let streak = 0, protecoes = 1, usadoEm = null;
  while (dias.has(cursor) || (streak > 0 && protecoes > 0 && streak % 7 !== 0)) {
    if (!dias.has(cursor)) { protecoes--; usadoEm = cursor; }
    streak++;
    const d = new Date(cursor + "T12:00:00Z"); d.setUTCDate(d.getUTCDate() - 1);
    cursor = d.toISOString().slice(0, 10);
  }
  return { disponivel: protecoes > 0, usadoEm };
}

// ---------------- 4.1 — vida útil da mistura (~14 noites) ----------------
export function estadoMistura(s) {
  if (!s.receitaPreparadaEm) return null;
  const base = s.misturaRenovadaEm || s.receitaPreparadaEm;
  const diasUso = diffDias(hojeSP(new Date(base)), hojeSP()) + 1; // 1 = dia do preparo
  const restantes = Math.max(0, 14 - diasUso + 1);
  return { diasUso, restantes, acabando: diasUso >= 12 };
}

export function renovarMistura(s) {
  s.misturaRenovadaEm = new Date().toISOString();
  s.pontos += 10; // recompensa por manter o ritual abastecido
  return save(s);
}
...
// 3.7 — foto "agora" para comparação Antes × Agora
export function salvarFotoAgora(s, dataURL) {
  s.fotoAgora = dataURL || null;
  s.fotoAgoraEm = dataURL ? new Date().toISOString() : null;
  return save(s);
}
...
export function salvarFotoPerfil(s, dataURL) {
  s.fotoPerfil = dataURL || null;
  return save(s);
}
...
export { FRASES_MANHA } from "./frases";
import { FRASES_MANHA } from "./frases";
